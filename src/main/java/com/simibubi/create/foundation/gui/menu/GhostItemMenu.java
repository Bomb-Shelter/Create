...
public abstract class GhostItemMenu<T, D> extends MenuBase<T, D> implements IClearableMenu {

	public ItemStackHandler ghostInventory;

	protected GhostItemMenu(MenuType<?> type, int id, Inventory inv, D extraData, boolean isExtraData) {
		super(type, id, inv, extraData, isExtraData);
	}
		...
		if (index < 36) {
			...
			for (int i = 0; i < ghostInventory.getSlotCount(); i++) {
				...
				if (stack.isEmpty()) {
					...
					copy.setCount(1);
					CreateTransferUtil.insertItem(ghostInventory.getSlot(i), copy, false);
					getSlot(i + 36).setChanged();
					break;
				}
			}
		} else {
			CreateTransferUtil.extractItem(ghostInventory.getSlot(index - 36), 1, false);
